...
export class InitialDatabase1596699016174 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "mutation_qc" (mutation_id SERIAL NOT NULL, sample_id integer NOT NULL, gene_name character varying, HGVS.c character varying, HGVS.p character varying, cosmic character varying, chr character varying, position character varying, qc number, CONSTRAINT mutation_pkey PRIMARY KEY (mutation_id), CONSTRAINT "foreign_mutation" FOREIGN KEY ("sample_id") REFERENCES public.samples (sample_id) MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE)`);
