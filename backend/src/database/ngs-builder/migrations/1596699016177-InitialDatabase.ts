...
export class InitialDatabase1596699016177 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS  "samples" ("sample_id" SERIAL NOT NULL, "run_id" SERIAL NOT NULL, "sample_name" character varying, "disease_id" SERIAL NOT NULL, specimen_no character varying, patient_name character varying, patient_sex character varying, specimen_type character varying, specimen_status character varying, patient_birth date, note1 character varying, note2 character varying, note3 character varying, CONSTRAINT "samples_pkey" PRIMARY KEY ("sample_id") , CONSTRAINT "foreign_run" FOREIGN KEY ("run_id") REFERENCES public.runs (run_id) MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE, CONSTRAINT foreign_checker FOREIGN KEY (checker) REFERENCES health_care_workers(worker_id) ON DELETE SET NULL ON UPDATE CASCADE, CONSTRAINT foreign_confirmer FOREIGN KEY (confirmer) REFERENCES health_care_workers(worker_id) ON DELETE SET NULL ON UPDATE CASCADE, CONSTRAINT foreign_quality_manager FOREIGN KEY (quality_manager) REFERENCES health_care_workers(worker_id) ON DELETE SET NULL ON UPDATE CASCADE, samples ADD CONSTRAINT foreign_report_doctor FOREIGN KEY (report_doctor) REFERENCES health_care_workers(worker_id) ON DELETE SET NULL ON UPDATE CASCADE)`);
